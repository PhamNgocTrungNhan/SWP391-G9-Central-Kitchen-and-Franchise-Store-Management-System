...
export default function CreateProductionBatchPage() {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                if (isInsufficientMaterial) {
                    throw new Error(
                        `THIẾU NGUYÊN LIỆU:\n\n${errorMsg}\n\n` +
                        `Lưu ý: Hệ thống tính nguyên liệu cần thiết dựa trên công thức của sản phẩm.\n` +
                        `Ví dụ: Nếu công thức quy định 1 bánh cần 20,000g bột, thì làm ${batch.quantityPlanned} bánh cần ${batch.quantityPlanned * 20000}g.\n\n` +
                        `Giải pháp:\n` +
                        `1. Nhập thêm nguyên liệu vào kho (nút "Nhập nguyên liệu" ở trên)\n` +
                        `2. Hoặc giảm số lượng thực tế xuống thấp hơn\n` +
                        `3. Hoặc kiểm tra công thức có đúng không`
                    )
                }
    ...
    const handleCreateFromRow = async (row, mfgDate, expDate) => {
        ...
        if (String(row?.productType || '').toUpperCase() === 'RAW') {
            setError('Sản phẩm RAW không thể tạo mẻ sản xuất. Chỉ tạo mẻ cho thành phẩm hoặc bán thành phẩm.')
            return
        }
        ...
        try {
            ...
            const inventoryHint = statusWarning
                ? ` ${statusWarning}`
                : ' Mẻ đã vào IN_PROGRESS; kho bếp đã trừ nguyên liệu RAW theo công thức (SL kế hoạch). Backend chuyển đơn sang PROCESSING khi đủ mẻ cho tất cả sản phẩm.'
                                        ...
                                        <tr key={row.key}>
                                            ...
                                            <td className="px-4 py-3 text-right">
                                                <button
                                                    ...
                                                    title={isRaw ? 'RAW không tạo mẻ sản xuất' : ''}
                                                >
