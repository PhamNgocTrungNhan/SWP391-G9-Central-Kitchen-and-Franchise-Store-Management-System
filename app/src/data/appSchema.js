...
export const pageCatalog = {
  ...
  inventory: {
    ...
    description: 'Giao diện tồn kho hỗ trợ tồn tổng, tồn theo storeId, nhật ký tồn kho, nhập nguyên liệu từ nhà cung cấp và chuyển hàng theo orderId.',
    authorize: 'Authorization: Bearer <JWT> | Role chính: ADMIN, MANAGER',
    endpoints: [
      { method: 'GET', path: '/api/Inventory/stock' },
      { method: 'GET', path: '/api/Inventory/store/{storeId}' },
      { method: 'GET', path: '/api/Inventory/logs' },
      { method: 'POST', path: '/api/Inventory/import', body: '{ productId, quantity, supplierId }', note: 'quantity > 0; chỉ nhập RAW; supplier phải active' },
  ...
  internalOrders: {
    ...
    description: 'Không gian này bao gồm tạo đơn, lọc danh sách theo storeId và trạng thái, xem chi tiết, giao từng phần, cùng các thao tác hủy, duyệt, từ chối, trả hàng, xác nhận hoàn tất và cập nhật trạng thái. Trạng thái PARTIAL_SHIPPING cho phép giao nhiều lần trước khi lên SHIPPING khi đã giao đủ mục tiêu từng dòng.',
    authorize: 'Yêu cầu Authorization: Bearer <JWT> cho toàn bộ endpoint. Ship partial: ADMIN/SUPPLY_COORDINATOR/KITCHEN_STAFF. Confirm-completed & return: ADMIN/STORE_STAFF.',
    endpoints: [
      ...
      { method: 'POST', path: '/api/internal-orders/{orderId}/ship-partial', body: 'Array<{ productId, quantityToShip }>', note: 'quantityToShip trong khoảng 0.01..999999; cho phép ship nhiều lần ở PARTIAL_SHIPPING' },
      { method: 'PUT', path: '/api/internal-orders/{orderId}/cancel' },
      { method: 'PUT', path: '/api/internal-orders/{orderId}/confirm-completed', note: 'Chỉ hợp lệ khi paymentStatus=PAID và orderStatus thuộc SHIPPING hoặc PARTIAL_SHIPPING' },
      ...
      { method: 'PUT', path: '/api/internal-orders/{orderId}/return', body: 'ReturnOrderRequest', note: 'Cho phép khi orderStatus thuộc SHIPPING hoặc PARTIAL_SHIPPING' },
      { method: 'PUT', path: '/api/internal-orders/{orderId}/status', body: 'UpdateOrderStatusRequest', note: 'Transition có PARTIAL_SHIPPING: PROCESSING->(PRODUCED|PARTIAL_SHIPPING|SHIPPING), PRODUCED->(PARTIAL_SHIPPING|SHIPPING), PARTIAL_SHIPPING->(SHIPPING|COMPLETED|RETURNED), SHIPPING->(COMPLETED|RETURNED)' },
    ],
  },
  productionBatches: {
